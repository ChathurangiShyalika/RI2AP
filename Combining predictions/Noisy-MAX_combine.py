...
#Getting maximum probable prediction at each time step
max_prob_indices = final_df.groupby('time')['y'].idxmax()
df_max_prob = final_df.loc[max_prob_indices]
df_max_prob

df_max_prob['new_index'] = range(1, len(df_max_prob) + 1)
df_max_prob
...
final_df.to_csv("Nosecone Removed_linear_noisymax.csv")
